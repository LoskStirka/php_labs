...
class Date
{
    ...
    public function __construct(string $calculatingDate)
    {
        ...
        if (!preg_match("/^[0-9,]+$/", $calculatingDate) || strlen($calculatingDate) < 5) {
            echo ("Вы ввели неправильно!\n");
        }
        $arrDate = explode(",", $calculatingDate);
        if ($arrDate[2] < 1) {
            echo ("Вы ввели неправильно год!\n");
        } else {
            $this->year = $arrDate[2];
        }
        if ($arrDate[1] < 1 || $arrDate[1] > 12) {
            echo ("Вы ввели неправильно месяц!\n");
        } else {
        ...
        if ($arrDate[2] % 4 === 0 && (int)$arrDate[1] === 2) {
            if ($arrDate[0] > 29 || $arrDate[0] < 1) {
                echo ("Вы ввели неправильно день!\n");
                $goodDay = false;
            }
        }
...
echo ("{$date->minusDay(45)}") . "\n";
echo ("{$date->format("ru")}") . "\n";
echo ("{$date->format("en")}") . "\n";
echo ("{$date2->getDateOfWeek()}") . "\n";
